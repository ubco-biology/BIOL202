...
Later we'll learn how to formally check the second assumption. For now we'll assume both assumptions are met.  

* We'll now provide an appropriate graph to accompany our test, and interpret the graph
...
The following graph illustrates this for the *t* distribution associated with $df = 24$: 

```{r showt, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 4}
...
```{r tcrit1}
tcrit <- qt(alpha/2, df = d_f, lower.tail = FALSE)
```
...
There, we generated a table of summary statistics that included the rule-of-thumb confidence limits; now we can include the precisely calculated lower and upper 95% confidence limits. 
...
If we wanted to report the confidence interval on its own, we can get the necessary information from our newly created table, and type this inline code in our markdown text (NOT in a code chunk):
