...
And a new package `forcats` that helps manage categorical variables.  Install this if you don't already have it.
...
Let's convert the "treatment" variable into a "factor" type variable using the `as_factor` function from the `forcats` package:

```{r covert_var}
circadian$treatment <- as_factor(circadian$treatment)  # first change variable to type "factor"
```

Now let's capitalize the category names (or "levels"):

```{r rename_treat}
circadian$treatment <- fct_recode(circadian$treatment,
                Control = "control", Knee = "knee", Eyes = "eyes")
```
