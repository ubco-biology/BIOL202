...
**H~A~**: The proportion of damselfly battles in the population flown in the counter-clockwise direction is not 0.5 ($p_0 = 0.5$)

We'll set $\alpha = 0.05$.

Although our $H_0$ makes a specific statement about a proportion ($p_0$), our _test statistic_ will actually be the **number of battles that occurred in a counter-clockwise direction**.  Of course, we can re-express this number as a proportion, but convention is that we use the number (frequency) instead.  
