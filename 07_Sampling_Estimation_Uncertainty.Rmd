...
The "set.seed" code was updated on `r format(Sys.time(), '%B %d, %Y')`.
...
We will also need a new package called `infer`, so install that package as you [previously learned](#package_install), then load it:

```{r desc1_package2, message = FALSE}
library(infer)
```
...
Functions such as `sample` and `slice_sample` have a challenge to meet: they must be able to take samples that are as truly random as possible. On a computer, this is easier said then done; special algorithms are required to meet the challenge. R has some pretty good ones available, so we don't have to worry about it (though to be honest, they can only generate "pseudorandom" numbers, but that's good enough for our purposes!).
...
* use the `slice_sample` function to randomly sample 20 rows from the `genelengths` tibble
...
gene.means.n20 <- genelengths %>%
  rep_slice_sample(n = 20, reps = 10000) %>%
  summarise(
...
OK, so we see that the minimum and maximum values are 1864 (under the "p0" heading) and 9349 (under the "p100" heading). So this can inform our x-axis limits when constructing the histogram (note that you typically need to try out different maximum values for the y-axis limits).

Let's first remind ourselves what the true mean gene length is in the entire population of genes:
...
```{r fig.cap = "Approximate sampling distribution of mean gene lengths from the human genome (10000 replicate samples of size 20).", fig.width = 4, fig.height = 4}
...
Before interpreting the histogram, let's first calculate the mean of the 10000 sample means:
...
Note that the mean of the sample means is around `r round(mean(gene.means.n20$mean_length, na.rm = TRUE), 0)`, which is pretty darn close to the true population mean of `r round(mean(genelengths$size, na.rm = TRUE), 0)`. 
...
```{r fig.cap = "Approximate sampling distribution of mean gene lengths from the human genome (10000 replicate samples of size 50).", fig.width = 4, fig.height = 4}
...
**NOTE** This type of graph is not something you'll need to reproduce!

```{r compare_sampling_dists, message = FALSE, warnings = FALSE, fig.cap = "Approximate sampling distributions of mean gene lengths from the human genome, using sample sizes of 20 and 50 (10000 replicate samples in each distribution).", fig.width = 4, fig.height = 4}
gene.means.combined %>%
...
Now let's show the code for calculating the SEM for the mean calculated using values in the "size" variable from our "newsamp.n20" object:
...
You've seen all but the last lines of code before! Importantly, as we've seen before, the "Count" variable tallies the number of non-missing observations in the variable of interest (here, "size"). This is the sample size that we'll need for the calculation of the SEM. 
...
<div class="note">
**TIP**
It is a good idea to use the longer approach that reports the sample size ("Count"), mean, standard deviation, and SEM. Why? Because in this way you're reporting each value ("n", "s") that goes into the calculation of the SEM.
</div>

### Calculating the __rule of thumb__ 95% confidence interval {#confint_rough}
...
We can re-use code from the previous section for the calculations:
...
If you were asked to report the rule-of-thumb 95% confidence **interval**, then you'd use the output from your calculations above to report `r round(temp$Lower_95_CL, 3) < $\mu$ < round(temp$upper_95_CL, 3)`. 
