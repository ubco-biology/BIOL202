...
This book is \textbf{currently under development, with sections being added weekly}.
...
Pither, J. (2021). \emph{Tutorials for BIOL202: Introduction to Biostatistics}. \url{https://ubco-biology.github.io/BIOL202/index.html}
...
\textbf{Note}
The latest version of R is 4.1.1 (``Kick Things''). It is strongly recommended that you install this latest version, even if you have older versions already installed. \textbf{However} If you have an older operating system on your computer, you'll want to check compatibility of both R and RStudio. The RStudio website should detect your operating system and provide the appropriate download option automatically. The website with older versions of R is \href{https://cran.r-project.org/bin/macosx/}{here}, and older versions of RStudio is \href{https://www.rstudio.com/products/rstudio/older-versions/}{here}.

Follow the instructions below, and optionally you can view this YouTube video \href{https://youtu.be/2Sovzf6lVRo}{here} \emph{prior} to starting the steps.
...
\item
  ...
  \end{itemize}
\item
  Alternatively, in the Console pane type the following (\textbf{NOTE} never include this code within your R Markdown document; only install packages by typing directly in the Console):
\end{itemize}
...
For an example of an HTML (webpage) document that illustrates the effectiveness of literate programming, take a look at \href{https://ubco-biology.github.io/BIOL202/Manning_et_al_full_script.html}{this HTML output - a webpage} that accompanied a lab-based experiment examining the potential for freshwater diatoms to be successfully dispersed over long distances adhered to duck feathers.
...
\href{https://youtu.be/DNS7i2m4sB0}{webpage}.
...
Now we're going to ensure R Markdown works the way we want. A key functionality we need is being able to ``knit'' our report to PDF format. For this, we need to install the \texttt{tinytex} package \{\#install\_tinytex\}.
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.6947}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.3053}}@{}}
...
\midrule()
\endhead
\url{https://www.zoology.ubc.ca/~schluter/R/} & UBC zoology site
...
\# Reproducible Research \{\#repro\_research\} & \\
A key goal of our Biology program at UBC's Okanagan campus is to f & oster appreciation for reproducible research, and to equip students (and professors) with skills that will help them undertake reproducible research themselves. \\
In first year you learned from the Biology program's {[}introductory & Open Science learning modules{]}(\url{https://ubco-biology.github.io/OS-Introduction/}) that reproducible research studies are not as common as one might assume. One key reason for this is insufficient documentation of all the steps taken along the research workflow. Moreover, conducting reproducible research is extremely challenging - more than most scientists appreciate. See, for example, an incredible, recent case concerning ageing experiments with \emph{C. elegans} \href{https://www.nature.com/news/a-long-journey-to-reproducible-results-1.22478}{here}. \\
...
\#\# Computational reproducibility \{\#comp\_repro\} & \\
Almost all research, whether it's conducted in the field or in a l & ab, includes a substantial amount of work that's done on the computer. This includes data processing, statistical analyses, data visualization and presentation, and production of research outputs (e.g.~publications). Some research is of course exclusively conducted on computers. The bottom line is that computer-based work forms a key and substantive part of all research workflows. \\
Given that this work is done on computers (which are entirely cont & rollable), it should be able to be reproduced \textbf{exactly}. This is known as \textbf{computational reproducibility}: an independent researcher should be able to access all data and scripts from a study, run those scripts, and get exactly the same outputs and results as reported in the original study. \\
In this tutorial you'll start gaining relevant experience and skil & ls by producing a reproducible lab report. \\
& \\
...
1. Review the Biology department's {[}Procedures and Guidelines webp & age{]}(\url{https://ubco-biology.github.io/Procedures-and-Guidelines/}) description of how to \href{https://ubco-biology.github.io/Procedures-and-Guidelines/file-and-data-management.html}{manage files and directories}. This should take about \textbf{20 minutes} \\
\#\# An example BIOL202 workflow \{\#biol202\_workflow\} & \\
Now that you have reviewed the fundamentals of \textless a href=``\url{https://ub} & co-biology.github.io/Procedures-and-Guidelines/glossary\#File-and-data-management''\textgreater file and directory management, you should decide how to best organize and manage the work you do for BIOL202. Here we'll provide one example approach that most if not all of you will find useful. \\
...
Why OneDrive? As a UBC student, you receive 1TB of free storage! A & nd you also get peace of mind knowing that your files are secure and up-to-date (provided you have an internet connection), and that OneDrive has something called ``version control'', which saves old versions of files and allows you to see those versions if you wish, \textbf{so long as you maintain the same file name}. \\
& \\
Those instructions are entirely valid! However, when you have acce & ss to a \emph{version control} system, like OneDrive, it is better to \textbf{keep the file name the same}, rather than changing it each time you update it. For example, your markdown file (which as a ``.Rmd'' extension to the name) that you use for your tutorial work should maintain the same name throughout the term, rather than saving a new file each time you do substantive work on it. \\
...
If you haven't set up OneDrive yet, follow the instructions provid & ed at this \href{https://lthub.ubc.ca/guides/microsoft-onedrive-student-guide/}{UBC IT website}. \\
& \\
\#\# Directory structure \{\#dir\_structure\} & \\
We anticipate three general categories of work being undertaken: & \\
* Lecture work, including annotating lecture notes (e.g.~on PDFs o
* Tutorial work, including practicing what you learn in tutorials
* Lab assignment work, in which you answer questions using R and R & r PowerPoints) and writing study notes of your own (e.g.~using Word)
using RStudio and R Markdown, and commenting about tips, or tricky bits
Markdown and create a document for submission and grading \\
...
\#\# Steps to set up directories \{\#setup\_dirs\} & \\
* Having successfully installed OneDrive, you should see a ``OneDri
* In your OneDrive folder, create a root directory''BIOL202'' to ho
* Create a ``\_README.txt file for the root BIOL202 directory, as pe & ve'' folder on your computer
st all of your BIOL202 work
r instructions in the \href{https://ubco-biology.github.io/Procedures-and-Guidelines/readme-files-and-data-dictionaries.html}{Biology Procedures and Guidelines webpage}. \\
& \\
* Within the BIOL202 root directory, create three (sub)directories
- ``BIOL202\_lecture''
- ``BIOL202\_tutorials''
- ``BIOL202\_assignments''
* Create a ``\_README.txt'' file in each of the three sub-directories & : \\
The BIOL202 directory, and all its contents, will now sync regular & ly to your online OneDrive account, so that you can access your up-to-date files from any device upon which OneDrive is installed. \\
...
\#\# Lecture workflow \{\#lecture\_workflow\} & \\
The directory for your lecture work is now ready to house any lect & ure-related work that you do. For instance, if you wish to type up study notes in a Word document, you could call that file ``Pither\_BIOL202\_lecture-notes.docx''. Each time you add/edit/update it, OneDrive will keep old versions for you! \\
\#\# Tutorial workflow \{\#tutorial\_workflow\} & \\
How you manage your tutorial workflow comes down to personal prefe
I have created an R Markdown file that you can download and use fo & rence. The instructions provided below create an RStudio project in the ``BIOL202\_tutorials'' directory (see below), and then create a single R Markdown document, formatted to have sections (headers) for each tutorial you work on. This approach makes it easier to find all of your work in the R Markdown file, so long as it is formatted logically.
r this purpose, and we'll download that a bit later. \\
First, we need to set up an RStudio project. & \\
\#\# Create an RStudio Project \{\#create\_project\} & \\
...
To do this, open RStudio then select File \textgreater{} New Project. Then sele & ct the option to use an existing directory, and locate and select your tutorial directory. Provide a name for your project file, such as ``BIOL202\_tutorial'', then select OK. If it asks to open a new RStudio session, you can say yes. \\
...
\textgreater{} Updated 7:15pm Sept 9 & \\
The following code will create a directory called ``figures'' in you & r working directory \\
\texttt{dir.create("figures")} & \\
You should see the new folder appear in the files panel on the bot & tom right of RStudio. \\
Whenever you wish to generate figures in R, then export them as im & age files, for example, you can save them to this folder. We'll learn about this later. \\
Reading and writing files from / to directories requires that we c & an tell R exactly where to find those directories. That's where the handy package called \texttt{here} comes in. \\
\#\#\# The \texttt{here} package\{\#here\_package\} & \\
Now we'll install and load an R package called \texttt{here} that will he & lp with file and directory management and navigation. \\
\texttt{install.packages("here")} & \\
This is a helpful {[}blog post{]}(\url{http://jenrichmond.rbind.io/post/how} & -to-use-the-here-package/) about the utility of the \texttt{here} package. \\
Then we load the package: & \\
\texttt{r\ library(here)} & \\
When you load the package, the \texttt{here} function takes stock of wher & e your RStudio project file is located, and establishes that directory as the ``working directory''. It should return a file path for you. In future tutorials we'll make use of the \texttt{here} package when saving files. \\
\#\# Edit an R markdown file \{\#edit\_markdown\} & \\
In a previous \protect\hyperlink{intro_markdown}{tutorial} you learned how to creat & e an R Markdown file, but generally you won't need to do this in this course, because you'll be provided starter documents to work with. \\
I have created an R Markdown file that you can download and use fo & r starting your tutorial work. \\
To do this, copy and paste the following code into your command co & nsole in RStudio (the bottom left panel), and press return: \\
\texttt{r\ download.file("https://raw.githubusercontent.com/ubco-biology/BIOL} & 202/main/more/Example\_tutorial\_markdown.Rmd'', ``BIOL202\_tutorial\_markdown.Rmd'') \\
...
* Change the ``author'' information at the top of the document (in t
* Save your file by clicking the save icon in the top left, and be & he header) to your name.
sure to give it an appropriate name as per previous file-naming instructions \\
...
\#\# Components of an R Markdown file \{\#md\_components\} & \\
At this point, you should familiarize yourself with the components & of a markdown file. \\
* * * & \\
1. R basics & \\
Read through section 4.2 of this online {[}tutorial{]}(\url{https://rbasics} & .netlify.app/4-rmarkdown.html\#the-components-of-an-r-markdown-file) and watch the videos there too \\
* * * & \\
\#\# Interacting with Tutorial material \{\#interact\_tutorial\} & \\
Students have typically worked through tutorials by reading the ma & terial, typing helpful notes in their markdown document, and copying and pasting any R code into ``chunks'' within their R Markdown document, then running those chunks. \\
You can insert as many chunks as you'd like. Chunk insertion is ac & hieved by selecting the green ``+'' icon at top of the editor panel, and selecting ``R''. Or go to the ``Code'' menu drop down and ``Insert Chunk''. \\
...
\#\# Lab assignments workflow \{\#assign\_workflow\} & \\
You can repeat the same steps from earlier for creating a new {[}RSt & udio project{]}(\#create\_project) for your lab work. But this time when you create the project, specify your lab assignment directory ``BIOL202\_assignments'' as the location. \\
You can optionally create subdirectories also, perhaps one for eac & h of the lab assignments. \\
The key difference in the assignments workflow is that for each as & signment you'll start with a new R Markdown (Rmd) file, which you'll download from Canvas under the ``assignments'' link. The document will include the questions you are to answer, and you simply edit / add to the document as you answer them. \\
Instructions for this will come in a later tutorial. & \\
Next tutorial: Practicing completing a very short lab assignment u & sing R Markdown. \\
& \\
\# Preparing and formatting assignments \{\#prepare\_assignments\} & \\
This material provides instructions on how to prepare and format R & Markdown documents for assignments. \\
\#\# Open your assignment RStudio project \{\#open\_assign\_project1\} & \\
In a previous \protect\hyperlink{assign_workflow}{tutorial} it was suggested you se & t up a working root directory called ``BIOL202\_assignments'' to host all your assignment work. You may have also created sub-directories, one for each of the three assignments. Regardless, in your root ``BIOL202\_assignments'' directory locate the RStudio project file (it has a ``Rproj'' extension in the filename), and open it by double-clicking it. \\
\#\# Download the assignment Rmd file \{\#download\_assign\_rmd\} & \\
The assignment file will be available to you for download from Can & vas under the ``Assignments'' section. The file is actually an R Markdown file, and therefore has an ``Rmd'' extension. It is simply a text document (meaning it contains only text), but the syntax used therein is ``markdown'' syntax, and the file extension (Rmd) specifies it as an R Markdown file. You might recall reading about Markdown in the \href{https://ubco-biology.github.io/Procedures-and-Guidelines/markdown-1.html}{Biology Procedures and Guidelines document}. \\
Make sure you save the Rmd file into your root ``BIOL202\_assignment & s'' directory, or alternatively into the appropriate sub-directory. \\
A practice assignment has been set up on Canvas, so that you can g & et used to this procedure. It is located under ``Lab\_materials'' module. Download the file, and make sure it's saved into your root directory (where your RStudio project is). \\
\#\# Open the assignment Rmd file \{\#open\_rmd\_file\} & \\
In RStudio, look in the ``files'' pane (bottom right) and locate you & r downloaded assignment Rmd file. Then click it to open. \\
This Markdown document includes the assignment questions, and serv & es as a starter document for your assignment. \\
Once you have opened the document, you'll see the following text a & t the top: \\
\textbackslash begin\{figure\}
\includegraphics[width=11.67in]{./more/assign_header} \textbackslash caption\{Ass
\textbackslash end\{figure\} & ignment markdown file header\}\label{fig:unnamed-chunk-6} \\
Complete the following steps: & \\
* keeping the quotation marks, replace the ``Practice assignment'' t
* keeping the quotation marks, replace the ``Firstname Lastname, Se
* keeping the quotation marks, replace the''Due date'' with the due & ext with the current assignment name (unless you're doing the practice assignment, in which case do not change anything!)
ction YY'' with your own Firstname Lastname and section number (e.g.~'' Jason Pither, Section 04'')
date of the current assignment, e.g.~``November 3, 2021'' \\
& \\
* save the Rmd file using a new name (so select file \textgreater{} ``save as''), & according to appropriate \href{https://ubco-biology.github.io/Procedures-and-Guidelines/file-and-data-management.html}{file naming conventions}, such as ``Pither\_BIOL202\_practice.Rmd''. Recall that if you're saving this on OneDrive, or in your local directory that is synced to OneDrive, then your file will be \protect\hyperlink{onedrive}{version controlled}. \\
You're now ready to start working on your assignment, and doing {[}l & iterate programming{]}(\url{https://ubco-biology.github.io/Procedures-and-Guidelines/glossary.html}) using R Markdown. \\
& \\
\#\# What to include in your answers \{\#what\_include\_answer\} & \\
The general approach is to enter your text answers and associated & R chunks with code underneath each question. \\
& \\
& \\
\#\#\# Code chunk headers \{\#chunk\_headers\} & \\
Here's an example code chunk in which the \texttt{tidyverse} package is b & eing loaded. You'll notice some extra text in the header of the chunk. \\
& aption\{Code chunk header with message suppression\}\label{fig:unnamed-chunk-7} \\
...
After the comma is \texttt{message\ =\ FALSE}. This tells R markdown to NO & T display any messages associated with the R commands in this chunk when knitting the R Markdown document. For example, when loading some libraries R will provide a long list of messages in the process. When you ``knit'' the R Markdown document, R will run each R chunk in your document. By including the \texttt{message\ =\ FALSE} heading argument, you can avoid including this extra output in your knitted document. \\
& \\
\bottomrule()
\end{longtable}
...
\includegraphics[width=28.14in]{./more/figure_chunk_header}

\includegraphics[width=10.81in]{./more/example_histogram_practice}
...
\begin{figure}
\includegraphics[width=11.67in]{./more/assign_header} \caption{Assignment markdown file header with figure caption option}\label{fig:unnamed-chunk-10}
\end{figure}
...
The use of the ``\%\textgreater\%'' syntax is described in another tutorial (yet to be posted!).
...
\begin{figure}
\includegraphics[width=10.75in]{./more/excel_tidy} \caption{Tidy data in Excel}\label{fig:unnamed-chunk-11}
\end{figure}
...
If you're curious about data wrangling, the \texttt{dplyr} package, which is loaded with the \texttt{tidyverse} package, provides all the tools for data wrangling. Its associated cheatsheet is available \href{https://github.com/rstudio/cheatsheets/raw/master/data-transformation.pdf}{here}.
...
If the file you wish to import is located on the web, then we need to provide the ``URL'' (the web address) to the \texttt{read\_csv} function. For example, in \href{\%60\%60\%60\%20example_data\%20\%3C-\%20read_csv(\%22https://raw.githubusercontent.com/ubco-biology/BIOL202/main/data/example_data.csv\%22)\%20\%60\%60\%60}{Tutorial 9 section 6} you imported the ``students.csv'' dataset from the course GitHub website, as follows:
...
This code imports the data and stores it in a local object called ``students''. The object is called a ``tibble'', which you can think of as a special kind of spreadsheet. More information on ``tibbles'' can be found \href{https://r4ds.had.co.nz/tibbles.html}{here}.
...
\begin{figure}
\includegraphics[width=19.67in]{./more/files_pane_rawdata} \caption{View of files with rawdata folder}\label{fig:unnamed-chunk-15}
\end{figure}
...
\textbf{New tool}\\
Pipes or ``\%\textgreater\%'' are implemented as part of the \texttt{magrittr} package, which is loaded with the \texttt{tidyverse}. In brief, pipes allow us to string together a series of functions, and we'll use them frequently in tutorials.
...
\label{tab:unnamed-chunk-20}Data summary
...
\item
  The \texttt{n} in the last line actually a function that tallies the sample size or count of all observations in the present category or group, and then the \texttt{sum(n)} sums up the total sample size. Thus, \texttt{n\ /\ sum(n)} calculates the relative frequency (equivalent to the proportion) of all observations that are within the given category
\end{itemize}
...
For now, let's go ahead and use the \texttt{kable} function that comes with the \texttt{knitr} package, using the pipe approach:

(\#tab:vis\_tigertable)Frequency table showing the activities of 88 people at the time they were attacked and killed by tigers near Chitwan national Park, Nepal, from 1979 to 2006
...
The key argument to the \texttt{kable} function is the table object (which here we provide before the pipe), and the figure caption.
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-23-1.pdf}
\caption{\label{fig:unnamed-chunk-23}Bar graph showing the activities of 88 people at the time they were attached and killed by tigers near Chitwan national Park, Nepal, from 1979 to 2006}
\end{figure}
...
\begin{figure}
\includegraphics[width=27.17in]{./more/bar_graph_chunk} \caption{Example code chunk for producing a good bar graph}\label{fig:unnamed-chunk-24}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-25-1.pdf}
\caption{\label{fig:unnamed-chunk-25}Histogram of body mass (g) for 342 penguins}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-26-1.pdf}
\caption{\label{fig:unnamed-chunk-26}Histogram of body mass (g) for 342 penguins}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-27-1.pdf}
\caption{\label{fig:unnamed-chunk-27}Histogram of body mass (g) for 342 penguins}
\end{figure}
...
(\#tab:birds\_table)Frequency table showing the frequencies of each of four types of bird observed at a marsh habitat (N = 86)
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-29-1.pdf}
\caption{\label{fig:unnamed-chunk-29}Histogram of body mass (g) for 342 penguins}
\end{figure}
...
\begin{figure}
\includegraphics[width=18in]{./more/na_error} \caption{Error when functions encounter 'NA' values}\label{fig:unnamed-chunk-30}
\end{figure}
...
(\#tab:desc\_bodymass)Descriptive statistics of measurements of body mass (g) for 342 penguins
...
(\#tab:vis2\_malaria)Contingency table showing the incidence of malaria in female great tits in relation to experimental treatment
...
(\#tab:vis2\_malaria\_prop)Contingency table showing the relative frequency of malaria in female great tits in relation to experimental treatment
...
\begin{figure}
\includegraphics[width=29.25in]{./more/grouped_bar_graph} \caption{Example code chunk for producing a good grouped bar graph}\label{fig:unnamed-chunk-37}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-38-1.pdf}
\caption{\label{fig:unnamed-chunk-38}Grouped bar graph showing the incidence of malaria in female great tits in relation to control (N = 35) and Egg-removal (N = 30) treatment groups.}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-39-1.pdf}
\caption{\label{fig:unnamed-chunk-39}Relative frequency of the incidence of malaria in female great tits in relation to control (N = 35) and Egg-removal (N = 30) treatment groups.}
\end{figure}

Here's the code:

\begin{figure}
\includegraphics[width=36.44in]{./more/mosaic_graph} \caption{Example code chunk for producing a good mosaic plot}\label{fig:unnamed-chunk-40}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-41-1.pdf}
\caption{\label{fig:unnamed-chunk-41}Scatterplot of the association between bill length and depth among 151 Adelie penguins}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-42-1.pdf}
\caption{\label{fig:unnamed-chunk-42}Serotonin levels in the central nervous system of desert locusts that were experimentally crowded for 0 (control), 1, and 2 hours. N = 10 per treatment group.}
\end{figure}
...
\caption{\label{fig:unnamed-chunk-43}Violin plot showing the body mass (g) of male Adelie (N = 73), Chinstrap (N = 34), and Gentoo (N = 61) penguins.}
\end{figure}
...
\caption{\label{fig:unnamed-chunk-44}Violin plot showing the body mass (g) of male Adelie (N = 73), Chinstrap (N = 34), and Gentoo (N = 61) penguins. Density plots are provided in the margin.}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-45-1.pdf}
\caption{\label{fig:unnamed-chunk-45}Boxplot showing the body mass (g) of male Adelie (N = 73), Chinstrap (N = 34), and Gentoo (N = 61) penguins. Density plots are provided in the margin. Boxes delimit the first and third quartiles, and the middle line depicts the median. Whiskers extend to values beyond the 1st (lower) and 3rd (upper) quartiles, up to a maximum of 1.5 x IQR, and individual black points are outlying values.}
\end{figure}
...
\caption{\label{fig:unnamed-chunk-46}Violin plot showing the body mass (g) of male Adelie (N = 73), Chinstrap (N = 34), and Gentoo (N = 61) penguins. Boxplots are superimposed, with boxes delimiting the first and third quartiles, and middle line depicting the median. Whiskers extend to values beyond the 1st (lower) and 3rd (upper) quartiles, up to a maximum of 1.5 x IQR, and individual black points are outlying values.}
\end{figure}
...
\subsection{Interpreting stripcharts and boxplots}\label{interp_violin}}
...
\textbf{NOTE - Updated Oct 20, 2021}
...
\caption{\label{fig:unnamed-chunk-49}Approximate sampling distribution of mean gene lengths from the human genome (10000 replicate samples of size 20).}
\end{figure}
...
\caption{\label{fig:unnamed-chunk-51}Approximate sampling distribution of mean gene lengths from the human genome (10000 replicate samples of size 50).}
\end{figure}
...
\textbf{NOTE} This type of graph is not something you'll need to be able to do for assignments!
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-52-1.pdf}
\caption{\label{fig:unnamed-chunk-52}Approximate sampling distributions of mean gene lengths from the human genome, using sample sizes of 20 and 50 (10000 replicate samples in each distribution).}
\end{figure}
...
\textbf{IMPORTANT:}\\
If you were asked to report the rule-of-thumb 95\% confidence \textbf{interval}, then you'd use the output from your calculations above to report:\\
...
\caption{\label{fig:unnamed-chunk-56}Approximate null distribution for the damselfly study, with n = 20 trials, and probability of success = 0.5 in each trial. Data are from 100000 repetitions.}
\end{figure}
...
\textbf{NOTE}
In 2021 we will not be covering ``\(\chi\)\textsuperscript{2} Goodness of Fit tests'', which are also tests that apply when analyzing a single categorical variable. For an older tutorial on this, see this \href{https://people.ok.ubc.ca/jpither/modules/Goodness_of_fit_tests.html\#assumptions_of_the_(chi)2_gof_test}{webpage} (thought it may soon be inactive).
...
\textbf{NOTE}: We could have calculated and isolated the ``P-hat'' value for predatory birds all in one go, using the following code:
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-62-1.pdf}
\caption{\label{fig:unnamed-chunk-62}Probability of obtaining X successes out of 6 random trials, with probability of success = 1/6.}
\end{figure}

\textbf{Added October 17, 2021}
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-63-1.pdf}
\caption{\label{fig:unnamed-chunk-63}Probability of obtaining X successes out of 15 random trials, with probability of success = 1/6.}
\end{figure}
...
\textbf{This tutorial will be completed in 2022}
...
\caption{\label{fig:unnamed-chunk-66}Relative frequency of cancer among women randomly assigned to control (n = 19942) and aspririn (n = 19934) treatment groups.}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-71-1.pdf}
\caption{\label{fig:unnamed-chunk-71}Mosaic plot of bird predation on killifish having different levels of trematode parasitism. A total of 50, 45, and 46 fish were in the uninfected, lightly infected, and highly infected groups.}
\end{figure}
...
\textbf{NOTE added November 12, 2021: }
If the normal distribution assumption is not met, and no \href{https://ubco-biology.github.io/BIOL202/checking-assumptions-and-data-transformations.html}{data transformation} helps, then one can conduct a ``non-parametric'' test in lieu of the one-sample \emph{t}-test, including a ``Sign test'' or a ``Wilcoxon signed-rank test''. A tutorial on non-parametric tests is under development, but will not be deployed until 2022. Consult chapter 13 in the Whitlock \& Schluter text, and this \href{https://whitlockschluter3e.zoology.ubc.ca/RExamples/Rcode_Chapter_13.html\#new_methods_on_this_page}{website} for some R examples.
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-74-1.pdf}
\caption{\label{fig:unnamed-chunk-74}Frequency distribution of body temperature (degrees Farenheit) for 25 randomly chosen healthy people.}
\end{figure}
...
\textbf{NOTE: }
A straightforward way to calculate the confidence interval for the mean of a numeric variable is using the \texttt{t.test} function. For an example, see this \protect\hyperlink{stats_2samp}{tutorial section}.
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-76-1.pdf}
\caption{\label{fig:unnamed-chunk-76}The \emph{t} distribution for df = 24.}
\end{figure}

Here's the code to calculate the critical value of \emph{t}, and recall that we've defined \texttt{alpha} and \texttt{d\_f} above:

\textbf{NOTE: Updated Nov.~13, 2021}

Added ``p ='' to the first argument in the \texttt{qt} function below.
...
\begin{figure}
\includegraphics[width=27.81in]{./more/confint_inline_code} \caption{Example of inline markdown and R code for a confidence interval}\label{fig:unnamed-chunk-77}
\end{figure}
...
\textbf{UPDATED November 21, 2021: }
We have removed all reliance on the \texttt{dlookr} package, which was causing problems on some systems. The \texttt{normality} function from this package was originally used for conducting the Shapiro-Wilk test for normality. We now provide a different way to do this.
...
\textbf{UPDATED November 15, 2021: }
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-81-1.pdf}
\caption{\label{fig:unnamed-chunk-81}Histogram of the differences in antibody production rate before and after the testosterone treatment}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-82-1.pdf}
\caption{\label{fig:unnamed-chunk-82}Paired plot of antibody production rate before and after the testosterone treatment}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-84-1.pdf}
\caption{\label{fig:unnamed-chunk-84}Normal quantile plot of the differences in antibody production rate before and after the testosterone treatment (ln{[}mOD/min{]}) 10\^{}-3.}
\end{figure}
...
\label{tab:unnamed-chunk-85}Data summary
...
\caption{\label{fig:unnamed-chunk-88}Violin and boxplot of the heights of students with right (n = 106) and left (n = 48) dominant eyes. Boxes delimit the first to third quartiles, bold lines represent the group medians, and whiskers extend to 1.5 times the IQR. Points beyond whiskers are extreme observations.}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-89-1.pdf}
\caption{\label{fig:unnamed-chunk-89}Normal quantile plots of height for students with right (n = 106) or left (n = 48) dominant eyes.}
\end{figure}
...
A tutorial on non-parametric tests is under development, but will not be deployed until 2022. Consult chapter 13 in the Whitlock \& Schluter text, and this \href{https://whitlockschluter3e.zoology.ubc.ca/RExamples/Rcode_Chapter_13.html\#new_methods_on_this_page}{website} for some R examples.
...
Load the \texttt{tidyverse}, \texttt{knitr}, \texttt{naniar}, \texttt{dlookr}, \texttt{car}, \texttt{skimr}, and \texttt{janitor} packages:
...
And additionally the \texttt{kableExtra} package, which is required for producing nicely formatted tables. Install this if you haven't already, then load:
...
And lastly, we need the \texttt{broom} package, which may also be new for you, and if so, install it.
...
\textbf{TIP:}
Given that the output from \texttt{skim\_without\_charts} is almost always rather wide, and overflows the page width, we'll get into the habit of transposing the output, and presenting using the \texttt{kbl} command. See the \href{https://ubco-biology.github.io/BIOL202/creating-tables-in-r-markdown.html}{tutorial on tables in markdown}

Let's save the skim output to an object:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{circ.skim }\OtherTok{\textless{}{-}}\NormalTok{ circadian }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{skim\_without\_charts}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

Then transpose using the \texttt{t} function, wrapped in the \texttt{kbl} function:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{kbl}\NormalTok{(}\FunctionTok{t}\NormalTok{(circ.skim))}
\end{Highlighting}
\end{Shaded}
...
This is just the same old output from \texttt{skim\_withouth\_charts} but here re-oriented to fit more easily on the page.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{unique}\NormalTok{(circadian}\SpecialCharTok{$}\NormalTok{treatment)}
\end{Highlighting}
\end{Shaded}
...
Let's first convert the ``treatment'' variable into a ``factor'' type variable using the \texttt{as.factor} base function:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{circadian}\SpecialCharTok{$}\NormalTok{treatment }\OtherTok{\textless{}{-}} \FunctionTok{as.factor}\NormalTok{(circadian}\SpecialCharTok{$}\NormalTok{treatment)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{levels}\NormalTok{(circadian}\SpecialCharTok{$}\NormalTok{treatment) }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"Control"}\NormalTok{, }\StringTok{"Eyes"}\NormalTok{, }\StringTok{"Knees"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{circadian}\SpecialCharTok{$}\NormalTok{treatment }\OtherTok{\textless{}{-}} \FunctionTok{ordered}\NormalTok{(circadian}\SpecialCharTok{$}\NormalTok{treatment, }\AttributeTok{levels =} \FunctionTok{c}\NormalTok{(}\StringTok{"Control"}\NormalTok{, }\StringTok{"Knees"}\NormalTok{, }\StringTok{"Eyes"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{verbatim}
## [1] Control Knees   Eyes   
## Levels: Control < Knees < Eyes
\end{verbatim}

OK, now we're ready to proceed with ANOVA!

\textbf{TIP:}
This procedure (ensuring the explanatory categorical variable is coded as a factor variable, and the ordering of the categories is correct) should also be done for 2-sample \emph{t}-tests, \textbf{if} there is an optimal ordering to the two groups.
...
\textbf{Additional steps are required for ANOVA tests}
...
\textbf{NOTE: }
Here we'll use what we learned in the \href{https://ubco-biology.github.io/BIOL202/creating-tables-in-r-markdown.html}{Tables in markdown tutorial} on creating nicely formatted tables, including \textbf{table headings}.

Please consult the \href{https://ubco-biology.github.io/BIOL202/creating-tables-in-r-markdown.html}{Tables in Markdown} tutorial for more details.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{kbl}\NormalTok{(circadian.stats, }\AttributeTok{caption =} \StringTok{"Descriptive statistics for the circadian rythm experiment data"}\NormalTok{, }
...
\caption{\label{tab:givenicepdftable2}Descriptive statistics for the circadian rythm experiment data}
\centering
\resizebox{\linewidth}{!}{
\begin{tabular}[t]{lrrrrrrr}
\toprule
treatment & Count & Count\_NA & Mean & SD & SEM & Low\_95\_CL & Up\_95\_CL\\
\midrule
Control & 8 & 0 & -0.31 & 0.618 & 0.218 & -0.825 & 0.208\\
Knees & 7 & 0 & -0.34 & 0.791 & 0.299 & -1.067 & 0.396\\
Eyes & 7 & 0 & -1.55 & 0.706 & 0.267 & -2.205 & -0.898\\
\bottomrule
\end{tabular}}
\end{table}
...
\hypertarget{visualizing-the-data-when-group-sample-sizes-are-large}{%
\subsubsection{Visualizing the data when group sample sizes are large}\label{visualizing-the-data-when-group-sample-sizes-are-large}}

Recall that we are focusing here on \textbf{Fixed-effects ANOVA} which generally apply to experiments. Experiments often don't have very large sample sizes per group, but if they do, we can use a different graph to visualize the data.
...
\NormalTok{students }\SpecialCharTok{\%\textgreater{}\%} 
  ...
  \FunctionTok{stat\_summary}\NormalTok{(}\AttributeTok{fun =}\NormalTok{ mean, }\AttributeTok{geom =} \StringTok{"point"}\NormalTok{, }
    \AttributeTok{colour =} \StringTok{"black"}\NormalTok{, }\AttributeTok{size =} \DecValTok{3}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{xlab}\NormalTok{(}\StringTok{"Dominant eye"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{ylab}\NormalTok{(}\StringTok{"Height (cm)"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_bw}\NormalTok{()}
\end{Highlighting}
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-95-1.pdf}
\caption{\label{fig:unnamed-chunk-95}Violin plot of heights of students with right (n = 106) and left (n = 48) dominant eyes. Solid circles denote group means, and bars +/- one standard error.}
\end{figure}
...
We see that the \emph{P}-value for the test is much greater than 0.05, so the null hypothesis of equal variance is not rejected. There is no evidence against the assumption that the variances are equal.
...
\textbf{NOTE}: These instructions are for a so-called \textbf{one-way ANOVA}, in which there is only one categorical variable against which a numerical variable is being analyzed. In the present case, we are analyzing a response variable in relation to the ``treatment'' categorical variable, which has 3 categories or ``levels''.

\hypertarget{createanovatable}{%
\subsection{Generate nice ANOVA table}\label{createanovatable}}
...
\textbf{NOTE}: this is a quick-and-dirty function that does \textbf{NOT} follow all the best practices in coding!
...
## 3 Total     16.64     21 NA      NA     NA
\end{verbatim}

It worked!

\textbf{Generate nice output using \texttt{kable}}

To generate a nice table that will export to PDF properly, we'll follow what we learned in the \href{https://ubco-biology.github.io/BIOL202/creating-tables-in-r-markdown.html}{Tables in markdown tutorial} on creating nicely formatted tables, including \textbf{table headings}.
...
\FunctionTok{kbl}\NormalTok{(nice.anova.table, }\AttributeTok{caption =} \StringTok{"ANOVA table for the circadian rythm experiment."}\NormalTok{, }
    \AttributeTok{booktabs =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{digits =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{4}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{4}\NormalTok{, }\DecValTok{4}\NormalTok{, }\DecValTok{3}\NormalTok{), }\AttributeTok{align =} \StringTok{"r"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%} 
...
\caption{\label{tab:niceanovapdf}ANOVA table for the circadian rythm experiment.}
\centering
\begin{tabular}[t]{rrrrrr}
\toprule
Source & SS & df & MS & F & P\_val\\
\midrule
treatment & 7.2245 & 2 & 3.6122 & 7.2894 & 0.004\\
...
\tightlist
\item
  the first line of code {[}options(knitr.kable.NA = '\,'){]} tells the \texttt{kbl} function to hide any ``NA'' values; we need this because parts of our table have NAs
\item
  the argument ``align = `r'\,'' in the \texttt{kbl} function tells it to align the text right
\end{itemize}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{circadianTukey.tidy }\OtherTok{\textless{}{-}} \FunctionTok{tidy}\NormalTok{(circadianTukey)}
\end{Highlighting}
\end{Shaded}
...
## 3 treatment Eyes-Knees             0 -1.216    -2.172    -0.2598    0.01168
\end{verbatim}
...
Now we can produce a nice output, which \textbf{is entirely optional}:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{kbl}\NormalTok{(circadianTukey.tidy.table, }
    \AttributeTok{caption =} \StringTok{"Results of a Tukey{-}Kramer post{-}hoc test for the circadian rythm experiment."}\NormalTok{, }
    \AttributeTok{booktabs =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{digits =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{3}\NormalTok{, }\DecValTok{4}\NormalTok{, }\DecValTok{4}\NormalTok{, }\DecValTok{3}\NormalTok{), }\AttributeTok{align =} \StringTok{"r"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{kable\_styling}\NormalTok{(}\AttributeTok{latex\_options =} \StringTok{"hold\_position"}\NormalTok{, }\AttributeTok{position =} \StringTok{"center"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\caption{\label{tab:niceTukeytable}Results of a Tukey-Kramer post-hoc test for the circadian rythm experiment.}
\centering
\begin{tabular}[t]{rrrrr}
\toprule
Contrast & Difference & Conf\_low & Conf\_high & Adj\_Pval\\
\midrule
Knees-Control & -0.027 & -0.9525 & 0.8986 & 0.997\\
Eyes-Control & -1.243 & -2.1682 & -0.3171 & 0.008\\
Eyes-Knees & -1.216 & -2.1716 & -0.2598 & 0.012\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{quote}
Shifts in circadian rhythm differ significantly among treatment groups (ANOVA; Table \ref{tab:niceanovapdf}; \emph{R}2 = 0.43). As shown in Figure \ref{fig:stripanovatukey}, the mean shift among the ``Eyes'' subjects was significantly lower than both of the other treatment groups.
\end{quote}
...
If the \textbf{equal-variance assumption} is violated for ANOVA, then you can set the ``var.equal'' argument to ``FALSE'' in the \texttt{t.test} function, in which case the function implements a ``Welch's \emph{t}-test''.
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-99-1.pdf}
\caption{\label{fig:unnamed-chunk-99}The association between inbreeding coefficient and number of surviving wolf pups (n = 24).}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{wolf.cor.tidy }\OtherTok{\textless{}{-}} \FunctionTok{tidy}\NormalTok{(wolf.cor)}
...
## 1  -0.6077    -3.589 0.001633        22  -0.8120   -0.2707 Pearson's pr~ two.si~
...
Let's visualize the association, again using the \texttt{geom\_jitter}1 function to help see overlapping values:
...
\begin{verbatim}
## Warning in cor.test.default(x = trick$years, y = trick$impressivenessScore, :
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{trick.cor.tidy }\OtherTok{\textless{}{-}} \FunctionTok{tidy}\NormalTok{(trick.cor)}
...
\addlinespace
factor.n\_unique & 3 & 3 & 2 &  &  &  &  & \\
factor.top\_counts & Ade: 152, Gen: 124, Chi: 68 & Bis: 168, Dre: 124, Tor: 52 & mal: 168, fem: 165 &  &  &  &  & \\
...
\caption{\label{tab:unnamed-chunk-100}Overview of the 'penguins' dataset}
\centering
...
\hypertarget{checking-assumptions-and-data-transformations}{%
\chapter*{Checking assumptions and data transformations}\label{checking-assumptions-and-data-transformations}}
\addcontentsline{toc}{chapter}{Checking assumptions and data transformations}
...
\caption{\label{fig:unnamed-chunk-107}Example of a normal quantile plot for a variable that is normally distributed.}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-108-1.pdf}
\caption{\label{fig:unnamed-chunk-108}The frequency distribution of the `biomass ratio' of 32 marine reserves.}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-109-1.pdf}
\caption{\label{fig:unnamed-chunk-109}Normal quantile plot of the `biomass ratio' of 32 marine reserves.}
\end{figure}
...
\textbf{Important: }
In a previous version of this tutorial we used the ``dlookr'' package and its \texttt{normality} function to test for normality, as follows
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-110-1.pdf}
\caption{\label{fig:unnamed-chunk-110}The frequency distribution of the `biomass ratio' of 32 marine reserves.}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-111-1.pdf}
\caption{\label{fig:unnamed-chunk-111}The frequency distribution of the `biomass ratio' of 32 marine reserves (log-transformed).}
\end{figure}
...
\caption{\label{fig:unnamed-chunk-112}Figure: Normal quantile plot of the `biomass ratio' of 32 marine reserves (log-transformed).}
\end{figure}
...
\caption{\label{fig:unnamed-chunk-114}Normal quantile plot of made-up biomass data.}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-119-1.pdf}
\caption{\label{fig:unnamed-chunk-119}Normal quantile plot of made-up biomass data, log-transformed.}
\end{figure}
...
\caption{\label{fig:unnamed-chunk-120}Normal quantile plot of the proportion of seeds fertilized on 30 plants (left) and the corresponding normal quantile plot (right)}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{BIOL202_files/figure-latex/unnamed-chunk-121-1.pdf}
\caption{\label{fig:unnamed-chunk-121}Normal quantile plot of the proportion of seeds fertilized (logit transformed) on 30 plants}
\end{figure}
...
\hypertarget{filename-directory-name-or-volume-label-syntax-is-incorrect}{%
\section*{Filename, directory name, or volume label syntax is incorrect}\label{filename-directory-name-or-volume-label-syntax-is-incorrect}}
\addcontentsline{toc}{section}{Filename, directory name, or volume label syntax is incorrect}

\begin{figure}
\centering
\includegraphics{/images/fig-cap-no-space1.png}
\caption{im}
\end{figure}

\textbf{Solution:} This error occurs because there is a mistake in the code for the figure caption. Specifically, there needs to be a space between the ``r'' and ``fig.'' in the RChunk header. See the screenshot below for the correct figure caption code.

\includegraphics{./more/fig-cap-no-space2.png}
