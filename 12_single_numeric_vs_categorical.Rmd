# Analyzing numerical response variable in relation to categorical variable {#numeric_categorical}
...
Here we use a method that relies on the data being stored in tidy (long) format, and assuming the order of the observations for each pair is correct.  For instance, recall our original "blackbird" tibble:

```{r showblackbird}
blackbird
```

We can see that birds 1 through 13 are ordered the same within each of the "After" and "Before" groups.  This is crucial for this implementation of the paired *t*-test.
...
* we run the `t.test` function with the arguments as follows: 
  ...
  + then we have an argument "data = .", which tells the `t.test` function that whatever data was passed to it from the preceding line is what will be used
...
Here's an example of a reasonable concluding statement, based on either of the 2 methods used above (note that in either case we call the test a "paired *t*-test, even if we used the one-sample *t*-test on the differences):

> We have no reason to reject the null hypothesis that the mean change in antibody production after testosterone implants was zero (paired _t_-test; _t_ = `r round(blackbird.paired.ttest$statistic,2)`; df = `r blackbird.paired.ttest$df`; _P_ = `r round(blackbird.paired.ttest$p.value,3)`; 95% confidence interval: `r round(blackbird.paired.ttest$conf.int[1], 3)` $< \mu_d <$ `r round(blackbird.paired.ttest$conf.int[2], 3)`). 
...
### Hypothesis statement {#2samp_hyp}
...
When we are analyzing a numeric response variable in relation to a categorical variable with two or more categories, it's good practice to provide a table of summary (or "descriptive") statistics, including confidence intervals.
...
height.stats <- students %>%
  ...
  summarise(
    ...
    SEM = SD_height/sqrt(Count),
    Lower_95_CL = t.test(height_cm, conf.level = 0.95)$conf.int[1],
    Upper_95_CL = t.test(height_cm, conf.level = 0.95)$conf.int[2]
  )
```
...
* we use the `t.test` function to calculate the lower and upper confidence limits. Specifically:
  + after the closing parenthesis to the `t.test` function, we include "$conf.int[1]", and this simply extracts the first value (lower limit) of the calculated confidence limit from the `t.test` output
...
### Assumptions of the 2-sample *t*-test {#2samp_assump}
...
> On average, students with left dominant eyes are similar in height to students with right dominant eyes (Figure ) (2-sample *t*-test; *t* = `r abs(round(height.ttest$statistic,2))`; df = `r height.ttest$df`; *P*-value = `r round(height.ttest$p.value,3)`; 95% confidence limits for the true difference in mean height: `r min(abs(round(height.ttest$conf.int[1], 3)),abs(round(height.ttest$conf.int[2], 3)))`, `r max(abs(round(height.ttest$conf.int[1], 3)),abs(round(height.ttest$conf.int[2], 3)))`).
