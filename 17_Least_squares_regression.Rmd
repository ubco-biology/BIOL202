...
(i) We wish to know whether some numeric variable _Y_ can be reliably predicted using a regression model of _Y_ on some predictor _X_.  For example, "Can vehicle mileage be reliably predicted by the weight of the vehicle?"  

(ii) We wish to test a specific hypothesis about the slope of the regression model of _Y_ on _X_.  For example, across species, metabolism (_q_) tends to scale with body mass (_M_) according to a(_q_ = _M_^{3/4}, which, when converted to a logarithmic relationship, gives a familiar form log(q) = 0.75*log(M). In this case, 0.75 is the empirically derived slope

In least-squares regression analysis, the "default" null hypothesis is that the _slope_ of the relationship between Y and X is equal to zero. That is:  
