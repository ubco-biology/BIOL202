...
Load the `tidyverse`, `skimr`, `broom`, `knitr`, and `visreg` packages.

The latter package is likely new to you. If so, install it first by typing the following in the command console:  

```
install.packages("visreg", dependencies = T)
```
...
<div class="note">
One need not use $\beta$ = 0 as the null hypothesis. For instance, research on allometry often tests null hypotheses about regressions slopes that differ from zero.  Curious? Check out this [website](https://www.nature.com/scitable/knowledge/library/allometry-the-study-of-biological-scaling-13228439/). 
</div>
...
* Assumptions  
    ...
    + if assumptions can't be met (e.g. after transformation), use alternative methods (**these alternative methods are not covered here**)
...
We can use the `broom` package and its `tidy` function to tidy up the regression model object:

```{r LSR_tidy_output}
biomass.lm.tidy <- tidy(biomass.lm)
```

Now have a look at this tidy output:
...
<div class="note">
The `plot.lm` function in the base stats package can be used on the regression output object to produce a series of so-called "regression diagnostic plots".  See the help file for info.
</div>
...
**NOTE**: if there are missing values in either of the X or Y variables used in the regression, then assigning residuals to a new variable within the original dataframe will not work, because the length of the "residuals" output will differ from the number of rows in the original dataframe.  

For instance, in the present example, there are no missing values, so the length of the residuals output will be the same as the number of rows in the original dataframe:

```{r}
length(residuals(biomass.lm))
nrow(plantbiomass)
```

**If you have missing values** in either the X or Y variable in your regression, see the section a bit further down.

If there are NO missing values, then if you want you can assing the residuals to a new variable in the original dataframe, for use in making the necessary residual plots:
...
```{r fig.cap = "Figure 3: Residual plot from a regression of biomass stability on species number for 161 plots.", fig.width = 4.2, fig.height = 4}
  ggplot(data = plantbiomass, aes(x = nSpecies, y = biomass.lm.resids)) +
  geom_point(shape = 1) +
