...
```{r vis1_skim, tidy = TRUE, tidy.opts = list(width.cutoff = 60)}
penguins %>%
  skim_without_charts()
```
...
Hmm, that histogram is ok, but perhaps not giving enough detail about the distribution. Let's try a bin width of 250 g:
...
This histogram is OK too... it's really more of an art than science. The key point is to not have too few observations falling within each bin, and not to have too many; you want to get a sense of the overall frequency distribution of the data.  Regardless, whatever histogram you produce, you should be able to describe what you see...
...
4. **Histogram**: Create a histogram of the bill lengths of penguins in the `penguins` dataset, and include an appropriate figure caption. 
